...
namespace LaLiga.ServiceForExternalApi
{
    public class DataToMatch : Controller
    {
        ...
        public async Task<ActionResult<Match>> GenerateMatch(string home, string away, int externalLeague, int externalSeazon)
        {
            ...
            if (!_context.Matches.Where(a => a.HomeTeam.TeamName == home && a.AwayTeam.TeamName == away && a.League.LeagueSeazon == externalSeazon).Any())
            {
                ...
                if (leagueData != null && fixtureId != 0)
                {
                    ...
                    if (!_context.Leagues.Where(a => a.LeagueId == externalSeazon && a.LeagueName == leagueData.league.name).Any())
                    {
                        Match.League = _context.Leagues.Where(a => a.LeagueId == externalSeazon && a.LeagueName == leagueData.league.name).FirstOrDefault();
                    }
                    else
                    {
                        league.LeagueName = leagueData.league.name;
                        league.LeagueSeazon = externalSeazon;
                        Match.League = league;
                    ...
                    {
                        Match.HomeTeam = _context.Teams.Where(a => a.TeamName == home).FirstOrDefault();
                    ...
                    {
                        Match.AwayTeam = _context.Teams.Where(a => a.TeamName == away).FirstOrDefault();
            ...
            {
                Match = _context.Matches.Where(a => a.HomeTeam.TeamName == home && a.AwayTeam.TeamName == away && a.League.LeagueSeazon == externalSeazon).FirstOrDefault();
            }
            return Match;
        }
        private bool IsThereInContext(string name)
        {
            return _context.Teams.Where(a => a.TeamName == name).Any();
        }
    }
}
