...
namespace LaLiga.ServiceForExternalApi
{
    ...
    {
        private readonly ExternalApiLeagueInSeazonClient _apiLeagueInSeazon;
        ...
        public DataToMatch(ExternalApiLeagueInSeazonClient externalApiLeagueInSeazon,
        ...
        public async Task<ActionResult<Match>> GenerateMatch(string home, string away, int externalLeague, int externalSeazon)
        {
            ...
            if (!_context.Matches.Where(a => a.HomeTeam.Name == home && a.AwayTeam.Name == away && a.SeazonLeague.SeazonId == externalSeazon).Any())
            {

                var recordInLeague = await  _apiLeagueInSeazon.Get(externalLeague, externalSeazon);
            ...
            {
                Match = _context.Matches.Where(a => a.HomeTeam.Name == home && a.AwayTeam.Name == away && a.SeazonLeague.SeazonId == externalSeazon).FirstOrDefault();
            }
            return Match;
        }
